...
const { inspectors } = storeToRefs(inspectorStore)

interface Props {
  isOpen: boolean
  inspectorID: string // текущий тип
}
const props = defineProps<Props>()

const emit = defineEmits<{
  close: [],
  inspectorUpdated: [id: string, surname: string]
}>()
...
const handleType = (id: string, surname: string) => {
  if (id != props.inspectorID) {
    emit('inspectorUpdated', id, surname)
  } else {
    err("Ошибка выбора типа", "Этот инспектор уже выбран")
  }
}
...
<template>
  <div class="modal-container" :class="{ active: isOpen }" @click="handleClose">
    <div class="modal-content" @click.stop>
      ...
      <h1>Выберите инспектора</h1>
      <div class="modal-body">
        <button
          v-for="(s, i) in inspectors"
          :key="i"
          class="role-item"
          :class="{disabled: s.id === props.inspectorID}"
